...
public class MixingFragment extends Fragment {
    ...
    // 异步任务引用
    private GetMaterialsTask getMaterialsTask;
    private SubmitMixingRecordTask submitMixingRecordTask;
    ...
    private void searchMaterials() {
        ...
        // 取消之前的异步任务
        if (getMaterialsTask != null && !getMaterialsTask.isCancelled()) {
            getMaterialsTask.cancel(true);
        }

        // 调用API获取原料列表
        getMaterialsTask = new GetMaterialsTask();
        getMaterialsTask.execute(semiProductCode);
    }
    ...
    private void openBucket(MaterialInfo material) {
        ...
        // 取消之前的异步任务
        if (submitMixingRecordTask != null && !submitMixingRecordTask.isCancelled()) {
            submitMixingRecordTask.cancel(true);
        }

        // 提交拌料记录
        submitMixingRecordTask = new SubmitMixingRecordTask();
        submitMixingRecordTask.execute(etSemiProductCode.getText().toString().trim(), material.getMaterialCode(), material.getBucketCode(), material.getWeight(), material);
    }
            ...
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    if (jsonArray.length() > 0 && !isCancelled()) {
                        ...
                        final String semiProductDesc = firstObject.optString("RawMaterialDesc", "");
